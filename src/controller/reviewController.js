...
const createReview = async (req, res) => {
  try {
    ...
    const countPartner = await ReviewModel.find({
      partnerId: partnerId,
      isDeleted: false,
    });
    if (!countPartner) {
    ...
    const saveAverageRating = await PartnerModel.findOneAndUpdate(
      { _id: partnerId, isDeleted: false },
...
const deleteReview = async (req, res) => {
  try {
    ...
    const removed = await ReviewModel.findByIdAndUpdate(reviewId, {
      isDeleted: true,
    });
    if (!removed) {
